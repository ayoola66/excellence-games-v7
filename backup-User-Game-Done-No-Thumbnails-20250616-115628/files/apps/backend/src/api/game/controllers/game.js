...
module.exports = createCoreController('api::game.game', ({ strapi }) => ({
  ...
  async findOne(ctx) {
    ...
    try {
      ...
      // --------------------------------------------------------------
      // Refresh the full user record so we definitely have
      // subscriptionStatus (the sanitised ctx.state.user may omit it)
      // --------------------------------------------------------------
      let userData = null;
      if (user) {
        userData = await strapi.entityService.findOne('api::user.user', user.id, {});
      } else {
        const authHeader = ctx.request.headers['authorization'] || ctx.request.headers['Authorization'];
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
          // If the game is free, allow access without authentication
          if (game.status !== 'premium') {
            userData = null;
          } else {
            return ctx.forbidden('No auth token');
          }
        } else {
          const token = authHeader.split(' ')[1];
          const decoded = await strapi.plugins['users-permissions'].services.jwt.verify(token);
          if (!decoded || !decoded.id) return ctx.forbidden('Invalid token');
          userData = await strapi.entityService.findOne('api::user.user', decoded.id, {});
          user = { id: decoded.id };
        }
      }

      const isPremium = (userData?.subscriptionStatus || '').toLowerCase() === 'premium';
      if (game.status === 'premium' && !isPremium) {
        console.warn(`Premium access denied for user ${user?.id}. subscriptionStatus=${userData?.subscriptionStatus}`);
        return ctx.forbidden('This game requires a premium subscription');
      }

      // Get or initialise the user's progress for this game
      let gameProgress = {};
      if (userData) {
  ...
  async getCategoryQuestions(ctx) {
    ...
    try {
      ...
      // Refresh full user record to ensure we have subscriptionStatus
      let userData = null;
      if (user) {
        userData = await strapi.entityService.findOne('api::user.user', user.id, {});
      } else {
      ...
      const isPremium = (userData?.subscriptionStatus || '').toLowerCase() === 'premium';
      if (game.status === 'premium' && !isPremium) {
        console.warn(`Premium category access denied for user ${user?.id}. subscriptionStatus=${userData?.subscriptionStatus}`);
      ...
      return ctx.throw(500, 'Error retrieving questions');
    }
  },
