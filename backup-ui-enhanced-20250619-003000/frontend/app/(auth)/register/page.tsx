...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await strapiApi.register(formData.email, formData.password)
      // Then login with the new credentials
      await login(formData.email, formData.password)
      compatToast.success('Registration successful! Welcome to Elite Games!')
      router.push('/game')
    } catch (error) {
      console.error('Registration error:', error)
      compatToast.error('Registration failed. Please try again.')
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="rounded-md shadow-sm -space-y-px">
            ...
            <div className="relative">
              ...
              <PhoneIcon className="h-5 w-5 text-gray-400 absolute top-3 left-3" />
              <input
                id="phone"
                name="phone"
                type="tel"
