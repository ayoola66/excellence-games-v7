...
import { getUserAuthTokens, setUserAuthTokens, clearUserAuthTokens } from '@/lib/auth/tokenManager'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { accessToken, refreshToken } = getUserAuthTokens()
    ...
    try {
      ...
      const response = await strapiApi.post('/api/auth/refresh', {
        refreshToken,
      ...
      setUserAuthTokens(nextResponse, { 
        accessToken: newAccessToken, 
        refreshToken: newRefreshToken 
      })
      
      // Log successful token refresh
      console.log(`[AUTH] User token refreshed for user ID: ${sessionInfo.userId}`)
    ...
    if (error instanceof AuthError) {
      ...
      if (
        ...
        clearUserAuthTokens(nextResponse)
        return nextResponse
      }
